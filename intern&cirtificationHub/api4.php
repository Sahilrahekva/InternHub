            ...
            if ($http_code === 200) {
                ...
                if (isset($data['data']) && is_array($data['data'])) {
                    foreach ($data['data'] as $job) {
                        ...
                        // Parse to our format (JSearch has job_title, employer_name, job_description, job_posted_at_date, job_apply_link)
                        ...
                        $link = $job['job_apply_link'] ?? "https://careers.{$company}.com";  // Use API link or default

                        $status = (strtotime($start_date) >= strtotime($sevenDaysAgo)) ? 'Launching Now' : 'Active';

                        // Cache in DB
                        $insert_stmt = $pdo->prepare("
                            INSERT INTO programs (company, type, title, description, start_date, end_date, status, link, posted_date, source) 
                            VALUES (?, 'internship', ?, ?, ?, ?, ?, ?, ?, 'jsearch')
                            ON DUPLICATE KEY UPDATE description = VALUES(description), link = VALUES(link), updated_at = CURRENT_TIMESTAMP
                        ");
                        $insert_stmt->execute([
                            ...
                            $start_date, $end_date, $status, $link, $start_date
                        ]);
            ...
            if ($xml) {
                foreach ($xml->channel->item as $item) {
                    ...
                    if (stripos($title, 'free') !== false || stripos($title, 'certification') !== false) {
                        ...
                        $link = (string)$item->link ?? 'https://learn.microsoft.com';  // Use RSS link or default
                        $status = (strtotime($start_date) >= strtotime($sevenDaysAgo)) ? 'Launching Now' : 'Active';
